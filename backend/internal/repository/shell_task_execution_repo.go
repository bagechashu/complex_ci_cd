...
const (
	sqShellExecutionInsert = "INSERT INTO shell_task_execution (task_id, server_id, command_id, status, output, error_message, exit_code, started_at, completed_at, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
	sqShellExecutionSelect = "SELECT id, task_id, server_id, command_id, status, output, error_message, exit_code, started_at, completed_at, created_at, updated_at FROM shell_task_execution"
	sqShellExecutionUpdate = "UPDATE shell_task_execution SET status=?, output=?, error_message=?, exit_code=?, started_at=?, completed_at=?, updated_at=? WHERE id=?"
	sqShellExecutionDelete = "DELETE FROM shell_task_execution WHERE id=?"
	sqShellExecutionCount  = "SELECT COUNT(*) FROM shell_task_execution"
)
...
func (r *SQLiteShellTaskExecutionRepository) Create(ctx context.Context, execution *models.ShellTaskExecution) error {
	...
	result, err := r.db.ExecContext(ctx, sqShellExecutionInsert,
...
func (r *SQLiteShellTaskExecutionRepository) GetByID(ctx context.Context, id int) (*models.ShellTaskExecution, error) {
	...
	err := r.db.QueryRowContext(ctx, sqShellExecutionSelect+" WHERE id = ?", id).Scan(
...
func (r *SQLiteShellTaskExecutionRepository) List(ctx context.Context, offset, limit int) ([]*models.ShellTaskExecution, int, error) {
	var total int
	err := r.db.QueryRowContext(ctx, sqShellExecutionCount).Scan(&total)
	if err != nil {
		return nil, 0, err
	}

	rows, err := r.db.QueryContext(ctx, sqShellExecutionSelect+" ORDER BY created_at DESC LIMIT ? OFFSET ?", limit, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *SQLiteShellTaskExecutionRepository) ListByTask(ctx context.Context, taskID int, offset, limit int) ([]*models.ShellTaskExecution, int, error) {
	...
	rows, err := r.db.QueryContext(ctx, sqShellExecutionSelect+" WHERE task_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", taskID, limit, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *SQLiteShellTaskExecutionRepository) ListByServer(ctx context.Context, serverID int, offset, limit int) ([]*models.ShellTaskExecution, int, error) {
	...
	rows, err := r.db.QueryContext(ctx, sqShellExecutionSelect+" WHERE server_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", serverID, limit, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *SQLiteShellTaskExecutionRepository) Update(ctx context.Context, execution *models.ShellTaskExecution) error {
	...
	_, err := r.db.ExecContext(ctx, sqShellExecutionUpdate,
...
func (r *SQLiteShellTaskExecutionRepository) Delete(ctx context.Context, id int) error {
	_, err := r.db.ExecContext(ctx, sqShellExecutionDelete, id)
	return err
}
...
func (r *SQLiteShellTaskExecutionRepository) GetLatestByTaskAndServer(ctx context.Context, taskID, serverID int) (*models.ShellTaskExecution, error) {
	var e models.ShellTaskExecution
	err := r.db.QueryRowContext(ctx, sqShellExecutionSelect+
