...
func (r *DeploymentTargetRepository) Create(target *models.DeploymentTarget) (*models.DeploymentTarget, error) {
	result, err := r.db.Exec(
		"INSERT INTO deployment_target (app_id, env_id, cluster_id, k8s_namespace, k8s_deployment, container_name, registry_domain, image_repo, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func (r *DeploymentTargetRepository) GetByID(id int) (*models.DeploymentTarget, error) {
	...
	err := r.db.QueryRow(
		"SELECT id, app_id, env_id, cluster_id, k8s_namespace, k8s_deployment, container_name, registry_domain, image_repo, created_at, updated_at FROM deployment_target WHERE id = ?",
		id,
...
func (r *DeploymentTargetRepository) GetByAppEnvCluster(appID, envID, clusterID int) (*models.DeploymentTarget, error) {
	target := &models.DeploymentTarget{}
	err := r.db.QueryRow(
		"SELECT id, app_id, env_id, cluster_id, k8s_namespace, k8s_deployment, container_name, registry_domain, image_repo, created_at, updated_at FROM deployment_target WHERE app_id = ? AND env_id = ? AND cluster_id = ?",
...
func (r *DeploymentTargetRepository) Update(target *models.DeploymentTarget) error {
	target.UpdatedAt = time.Now()
	_, err := r.db.Exec(
		"UPDATE deployment_target SET app_id = ?, env_id = ?, cluster_id = ?, k8s_namespace = ?, k8s_deployment = ?, container_name = ?, registry_domain = ?, image_repo = ?, updated_at = ? WHERE id = ?",
...
func (r *DeploymentTargetRepository) Delete(id int) error {
	_, err := r.db.Exec("DELETE FROM deployment_target WHERE id = ?", id)
	if err != nil {
